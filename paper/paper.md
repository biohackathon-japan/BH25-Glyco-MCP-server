...
To implement these mappings in practice, we deployed the YAML schema within an MCP (Model Context Protocol) server to act as a controler between the LLM and and the GlyCosmos and PubChem API endpoints. Each endpoint was defined as an MCP tool, complete with arguments, documentation and expexted outputs. When a user posees a natural language question the MCP exposed the available YAML file to the LLM, whcih then selected the approached api and filled in itss paramters. This MCP server then executed the HTTP request, retreived the JSON response, and returned thes trucutered results to the LLM. This archteitrue ensusres that hte LLM does not "hallucinate query synatx, but is more focued on mapping intent to apis, and chaining valid responses. This allowed for a multi-setp workflow in a reproducible, modular, and easyily documenteiotne fashion.
...
# Discussion
These results demonstrate the potential of using MCP servers with LOLMS to acecss glycoinformatic rources across different databasesd. By using SPARLQlist endpoints as atmoic building blockers, we have deomnstratedthat LLMs can map natuarl language queries to structured, chained API calls. This modular approach appears to be more robust than relying on direct federated SPAQRL queries whic are burttle and idfficult fo LLMs to generte consistely.

This project also reaveld strucutral challenges in the GlyCOsmos ecosystem. Of the 373 SPARQList APIs, only a small fraction were usable in practice. This limited coverage contrains the range of use cases that can be support byu natual langauge mappings and indicates a need for documentaiton and mantainnce of the Glycosmos endpoins. Our aduit of working versus broken endpoitns already provides a readmod for addressing this sissue inthe future.

Another issue was the balance between LLm reaconing and MCP control. The MCP server effecitly prevents query hallucinations by provideing the LLM with a set of predfined queries, ensuring accuracy and precision of queries. However, The LLM still shows variability in handling vaug eo r underspecified queries. For example, questions such as "tell me about glycans that bind to influenza virus", where there dozens of glycans that bind to the influenza virus recorded in GlyCosmos. The LLM would query all glycans one by one, adding time xomplecity and uncessary cost to answering the question. 

Overall, thius project demonstrates that MCP based naturla lanauge access is feasible for glycobiology, briding the gap between researchs and programmatic access of Glycosmos and PubChem in the hopes of faciliting the query process of glyco-related data.
